...
export async function issueCode(opts: { clientId: string; ownerId: string; redirectUri: string; codeChallenge: string | null; scope: string | null }) {
  const code = 'sfa_' + randomBytes(24).toString('base64url');
  const exp = new Date(Date.now() + 10 * 60 * 1000);
  await sql!`INSERT INTO oauth_codes (code, client_id, owner_id, redirect_uri, code_challenge, scope, expires_at)
             VALUES (${code}, ${opts.clientId}, ${opts.ownerId}, ${opts.redirectUri}, ${opts.codeChallenge}, ${opts.scope}, ${exp})`;
  return code;
}
...
async function mintAccessToken(ownerId: string, clientName: string) {
  const key = 'sf_' + randomBytes(24).toString('base64url');
  await sql!`INSERT INTO api_keys (key, owner_id, label) VALUES (${key}, ${ownerId}, ${('OAuth: ' + clientName).slice(0, 60)})`;
  return key;
}
...
export async function exchange(params: URLSearchParams) {
  ...
  if (grant === 'authorization_code') {
    ...
    const access = await mintAccessToken(row.owner_id, client.client_name);
    const refresh = 'sfr_' + randomBytes(24).toString('base64url');
    await sql!`INSERT INTO oauth_refresh (token, client_id, owner_id) VALUES (${refresh}, ${clientId}, ${row.owner_id})`;
