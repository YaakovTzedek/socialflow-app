...
export async function GET(
  ...
  try {
    ...
    const comments = await listComments(params.postId, pageToken);
...
// POST /api/posts/:postId/comments  { pageId, message }  → comment on a post
export async function POST(
  ...
  try {
    ...
    const result = await commentOnPost(params.postId, message.trim(), pageToken);
